...
# Use the implementation lowerer directly, not the public compatibility wrapper.
# The public wrapper uses ValueFromRemainingArguments, which is fine for a human CLI
# call but can mis-bind typed switch/number arguments when another script forwards
# a generated argument list. In particular, OutDir can slide into HoldMilliseconds.
$lowerer = Join-Path $RepoRoot "Tools\Lowering\DX12\lower_m20e1_dx12_clear_from_ir.ps1"
if (-not (Test-Path $lowerer)) {
    throw "M20E1 lowerer implementation not found: $lowerer"
}
...
namespace
{
    // M31C_UI_STABLE_CLIENT_PIXEL_SPACE: UI geometry is lowered in logical
    // client pixels. Keep the native window DPI-aware and create a client rect
    // that matches the authored resolution; otherwise Windows title bars, DPI
    // virtualization, resize borders and maximized windows stretch the backbuffer
    // and make parented button text appear to drift between window modes.
    void EnableArqenDpiAwareness()
    {
        ...
        HANDLE perMonitorAwareV2 = reinterpret_cast<HANDLE>(static_cast<INT_PTR>(-4));
    ...
    DWORD ArqenFixedClientWindowStyle()
    {
        // M31D_UI_LETTERBOX_CLIENT_SPACE: keep the authored initial client
        // size, but allow normal resize/maximize. The DX12 runtime resizes the
        // backbuffer and renders the authored UI through an aspect-preserving
        // logical viewport instead of letting window size corrupt coordinates.
        return WS_OVERLAPPEDWINDOW;
    }

    SIZE ArqenOuterWindowSizeForClient(DWORD style, DWORD exStyle, LONG clientWidth, LONG clientHeight)
    {
    ...
    SIZE ArqenAuthoredOuterWindowSize()
    {
        return ArqenOuterWindowSizeForClient(
            ArqenFixedClientWindowStyle(),
            0,
            static_cast<LONG>(ARQEN_M20E1_WINDOW_WIDTH),
            static_cast<LONG>(ARQEN_M20E1_WINDOW_HEIGHT));
    }

    void ApplyArqenFixedClientSize(HWND hwnd)
    {
        if (!hwnd)
            return;
        RECT client = {};
        ...
        if (clientW == static_cast<LONG>(ARQEN_M20E1_WINDOW_WIDTH) && clientH == static_cast<LONG>(ARQEN_M20E1_WINDOW_HEIGHT))
            return;
        const SIZE outer = ArqenAuthoredOuterWindowSize();
        SetWindowPos(hwnd, nullptr, 0, 0, outer.cx, outer.cy, SWP_NOMOVE | SWP_NOZORDER | SWP_NOACTIVATE | SWP_NOOWNERZORDER);
    }
...
int main()
{
    ...
    const DWORD windowStyle = ArqenFixedClientWindowStyle();
    const SIZE outerSize = ArqenOuterWindowSizeForClient(
        windowStyle,
        exStyle,
        static_cast<LONG>(ARQEN_M20E1_WINDOW_WIDTH),
        static_cast<LONG>(ARQEN_M20E1_WINDOW_HEIGHT));
    ...
    HWND hwnd = CreateWindowExW(
        ...
        CW_USEDEFAULT,
        CW_USEDEFAULT,
        static_cast<int>(outerSize.cx),
        static_cast<int>(outerSize.cy),
        nullptr,
        nullptr,
        hInstance,
        nullptr);
    ...
    {
        char clientLog[160] = {};
        std::snprintf(clientLog, sizeof(clientLog), "M31D letterbox client pixels client=%ldx%ld authored=%dx%d",
    ...
    const ArqenDx12UiControl uiControls[] = ARQEN_M30A_UI_CONTROL_DATA;
    ...
    triangleDesc.enableUiOverlay = ARQEN_M30A_UI_OVERLAY_ENABLED != 0;
    triangleDesc.uiControls = uiControls;
    triangleDesc.uiControlCount = ARQEN_M30A_UI_CONTROL_COUNT;
...
try {
    $runtimeObj = Join-Path $objDir "ArqenDx12ClearWindow.obj"
    $generatedObj = Join-Path $objDir "m20e1_dx12_clear_generated_smoke.obj"
    $pdbPath = Join-Path $objDir "dx12_build.pdb"

    & cl.exe /nologo /std:c++20 /EHsc /W4 /DUNICODE /D_UNICODE /I$runtime /I$OutDir /Fo:$runtimeObj /Fd:$pdbPath /c $cpp
    if ($LASTEXITCODE -ne 0) {
        throw "MSVC compile failed for ArqenDx12ClearWindow.cpp."
    }

    & cl.exe /nologo /std:c++20 /EHsc /W4 /DUNICODE /D_UNICODE /I$runtime /I$OutDir /Fo:$generatedObj /Fd:$pdbPath /c $generatedSmoke
    if ($LASTEXITCODE -ne 0) {
        throw "MSVC compile failed for generated DX12 smoke source."
    }

    & cl.exe /nologo $runtimeObj $generatedObj /Fe:$out d3d12.lib dxgi.lib d3dcompiler.lib user32.lib gdi32.lib dwmapi.lib
    if ($LASTEXITCODE -ne 0) {
        throw "MSVC link failed."
    }

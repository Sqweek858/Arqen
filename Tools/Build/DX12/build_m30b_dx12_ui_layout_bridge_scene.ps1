...
function Fail-M30B { param([string]$Message) throw "M30B/M30C DX12 UI layout bridge scene failed: $Message" }

if ([string]::IsNullOrWhiteSpace($RepoRoot)) { $RepoRoot = (git rev-parse --show-toplevel).Trim() }
if ([string]::IsNullOrWhiteSpace($SourcePath)) { $SourcePath = Join-Path $RepoRoot "Samples\DX12\dx12_ui_layout_bridge_scene_m30b.arq" }
if ([string]::IsNullOrWhiteSpace($OutDir)) { $OutDir = Join-Path $RepoRoot "Build\M30B" }
...
try {
    ...
    foreach ($marker in @("UI_OBJECT", "UI_SET", "UI_LAYOUT", "UI_PARENT", "STYLE", "UI_EVENT", "DX12_OBJECT_SELECTOR", "DX12_DIRECTIONAL_LIGHT", "DX12_OBJECT_PRIMITIVE")) {
        if (-not $irText.Contains($marker)) { Fail-M30B "compiled IR does not contain $marker." }
    }
    foreach ($marker in @("name=InspectorPanel", "name=AnimationSwitch", "name=LightSwitch", "property=checked", "event=clicked", "child=PanelTitle", "parent=InspectorPanel", "kind=box", "projection=perspective")) {
        if (-not $irText.Contains($marker)) { Fail-M30B "compiled IR missing M30B/M30C marker: $marker" }
    }
    ...
    foreach ($marker in @("M30A_UI_OVERLAY|True", "M30A_UI_CONTROLS|3", "M30B_UI_LAYOUT_HYGIENE|True", "M30B_TEXT_CLIPPING|True", "M30C_UI_PARENT_CLIP_BRIDGE|True", "M30C_PARENT_RELATIVE_LAYOUT|True", "M30D_UI_CLICK_EVENT_BRIDGE|True", "M29C_OBJECT_SELECTOR|True", "M29_FAKE_LIGHTING|True")) {
        if (-not $manifest.Contains($marker)) { Fail-M30B "manifest missing marker: $marker" }
    }
    foreach ($marker in @("ARQEN_M30A_DX12_UI_OVERLAY 1", "ARQEN_M30A_UI_OVERLAY_ENABLED 1", "ARQEN_M30A_UI_CONTROL_COUNT 3", "ARQEN_M30B_TEXT_CLIPPING 1", "ARQEN_M30B_BUTTON_TEXT_CENTERING 1", "ARQEN_M30C_UI_PARENT_CLIP_BRIDGE 1", "ARQEN_M30D_UI_CLICK_EVENT_BRIDGE 1", "ARQEN_DX12_UI_ACTION_TOGGLE_ANIMATION", "ARQEN_DX12_UI_ACTION_TOGGLE_FAKE_LIGHT")) {
    ...
    if ($BuildNative -or $Run) {
        ...
        if ([string]::IsNullOrWhiteSpace($Renderer)) {
            & $builder -IrPath $irPath -RepoRoot $RepoRoot -OutDir $OutDir -ExeName "m30b_dx12_ui_layout_bridge_scene.exe" -RequireFrame -RequireTriangle -HoldMilliseconds $HoldMilliseconds -FrameCount $FrameCount -TargetFps $TargetFps -KeepOpen:$KeepOpen -Run:$Run
        } else {
            & $builder -IrPath $irPath -Renderer $Renderer -RepoRoot $RepoRoot -OutDir $OutDir -ExeName "m30b_dx12_ui_layout_bridge_scene.exe" -RequireFrame -RequireTriangle -HoldMilliseconds $HoldMilliseconds -FrameCount $FrameCount -TargetFps $TargetFps -KeepOpen:$KeepOpen -Run:$Run
        }
    }

    if (-not $Quiet) { Write-Host "PASS|m30b_dx12_ui_layout_bridge_scene|source=$SourcePath|out=$OutDir|keep_open=$([bool]$KeepOpen)|fps=$TargetFps" }
} finally {
    Pop-Location
}

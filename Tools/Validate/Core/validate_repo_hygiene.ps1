...
function Emit-Warn {
    ...
    $lines.Add("WARN|$Name|$Message") | Out-Null
...
$requiredTools = @(
    ...
    "Tools/Generate/generate_error_code_registry.ps1",
    ...
    "Tools/Validate/DX12/validate_m19a_runtime_loop_contract.ps1",
    "Tools/Validate/DX12/validate_m19b_style_contract.ps1",
    "Tools/Validate/DX12/validate_m19c_ui_contract.ps1",
    "Tools/Validate/DX12/validate_m19d_ui_layout_contract.ps1",
    "Tools/Validate/DX12/validate_m19efgh_ui_final_contract.ps1"
)
...
if (Test-NoCrLf ".gitignore") {
    Emit-Check "lf_pinned_gitignore_no_crlf" $true ".gitignore has LF line endings"
} else {
    Emit-Warn "lf_pinned_gitignore_no_crlf" ".gitignore has CRLF locally; Git normalization may fix this on checkout, and this is not a source/tooling failure"
}

...
static partial class Program
{
    ...
    static List<Dictionary<string, string>> OrderedActionMaps(IrModel ir)
        => OrderedActionMaps(ir, ir.EntryActions);

    static List<Dictionary<string, string>> OrderedActionMaps(IrModel ir, IEnumerable<string> actionIds)
        => actionIds
            .Where(id => ir.Actions.ContainsKey(id))
            .Select(id => ir.Actions[id])
            .ToList();

    static IEnumerable<Dictionary<string, string>> AllActionMaps(IrModel ir)
    {
        foreach (var action in OrderedActionMaps(ir))
            yield return action;
        foreach (var fn in ir.Functions.Values)
            foreach (var action in OrderedActionMaps(ir, fn))
                yield return action;
    }

    static bool HasFileIoActions(IrModel ir)
        => AllActionMaps(ir).Any(action =>
            action.TryGetValue("op", out var op) &&
            op is "function_call" or "function_call_assign" or "function_return" or "function_return_int" or "function_return_bool" or "function_return_string" or "file_write" or "file_append" or "file_load" or "print_runtime_slot" or "command_arg_count" or "command_arg_index" or "runtime_int_set" or "runtime_int_add" or "runtime_int_sub" or "runtime_bool_set" or "runtime_bool_not_set" or "runtime_bool_toggle" or "runtime_trap_if_bool_false" or "runtime_string_set" or "runtime_string_concat" or "runtime_string_substring" or "runtime_int_parse" or "runtime_if_int" or "runtime_if_bool" or "runtime_if_string" or "runtime_else" or "runtime_if_end" or "runtime_while_int" or "runtime_break" or "runtime_continue" or "runtime_while_end");

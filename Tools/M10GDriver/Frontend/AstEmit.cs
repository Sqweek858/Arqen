...
static partial class Program
{
    static IEnumerable<string> AstLines(AstModel ast)
    {
        ...
        foreach (var line in AstMessageLines(ast))
            yield return line;
        foreach (var fn in ast.RuntimeFunctions)
        {
            var paramText = fn.Params == null || fn.Params.Count == 0
                ? ""
                : string.Join(",", fn.Params.Select(p => p.Type + ":" + Esc(p.Name) + ":" + Esc(p.Slot) + (p.Size > 0 ? ":" + p.Size.ToString() : "")));
            var localText = fn.Locals == null || fn.Locals.Count == 0
                ? ""
                : string.Join(",", fn.Locals.Select(l => l.Type + ":" + Esc(l.Name) + ":" + Esc(l.Slot)));
            yield return $"FUNCTION|name={Esc(fn.Name)}|return={Esc(fn.ReturnType)}|params={Esc(paramText)}|locals={Esc(localText)}|actions={fn.Actions.Count}";
        }
    ...
    static IEnumerable<string> AstUiFinalLines(AstModel ast)
    {
        foreach (var ev in ast.UiEvents)
            yield return $"UI_EVENT|event={Esc(ev.Event)}|target={Esc(ev.Target)}|target_kind={Esc(ev.TargetKind)}|body_lines={ev.BodyLineCount}|body={Esc(ev.BodyText)}";

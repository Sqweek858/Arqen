...
static partial class Program
{
    static IEnumerable<string> IrLines(AstModel ast, string sourcePath)
    {
        ...
        if (ast.RuntimeActions.Count > 0)
        {
            ...
            for (var f = 0; f < ast.RuntimeFunctions.Count; f++)
            {
                var fn = ast.RuntimeFunctions[f];
                var ids = new List<string>();
                for (var i = 0; i < fn.Actions.Count; i++)
                {
                    var id = $"fn_{f}_act_{i}";
                    ids.Add(id);
                    yield return RuntimeIrActionLine(id, fn.Actions[i]);
                }
                yield return FunctionIrLine(fn.Name, fn.ReturnType, fn.Params, fn.Locals, ids);
            }
    ...
    static string UiEventIrLine(UiEvent ev)
        => $"UI_EVENT|event={Esc(ev.Event)}|target={Esc(ev.Target)}|target_kind={Esc(ev.TargetKind)}|body_lines={ev.BodyLineCount}|body={Esc(ev.BodyText)}";
    ...
    static string FunctionIrLine(string name, string returnType, List<RuntimeParam>? parameters, List<RuntimeLocal>? locals, List<string> actionIds)
    {
        var paramText = parameters == null || parameters.Count == 0
            ? ""
            : string.Join(",", parameters.Select(p => p.Type + ":" + Esc(p.Name) + ":" + Esc(p.Slot) + (p.Size > 0 ? ":" + p.Size.ToString() : "")));
        var localText = locals == null || locals.Count == 0
            ? ""
            : string.Join(",", locals.Select(l => l.Type + ":" + Esc(l.Name) + ":" + Esc(l.Slot)));
        return $"FUNCTION|name={Esc(name)}|return={Esc(returnType)}|params={Esc(paramText)}|locals={Esc(localText)}|actions={Esc(string.Join(",", actionIds))}";
    }

...
static partial class Program
{
    sealed partial class Parser
    {
        ...
        void ApplyNumericUpdate(Token target, ExprResult value, string op)
        {
            ...
            {
                ApplyRuntimeIntUpdate(target, current, value, op);
                return;
            }
        ...
        void ApplyRuntimeIntUpdate(Token target, VarInfo current, ExprResult value, string op)
        {
            if (current.Type != "int")
                throw new CompileError("SEMANTIC", "S134", target.Line, target.Column, $"Runtime math target \"{target.Value}\" must be a runtime int slot.");
            if (op != "+" && op != "-")
                throw new CompileError("SEMANTIC", "S136", target.Line, target.Column, "M34D runtime int math supports add/remove only.");

            if (TryRuntimeIntSymbolOperand(value, out var sourceName))
            {
                AddRuntimeAction(new RuntimeAction(op == "+" ? "runtime_int_add" : "runtime_int_sub", "", "slot", sourceName, RuntimeSlotName(target)));
                return;
            }

            if (!IsNumeric(value.Type) || Math.Abs(ToNumber(value) - Math.Round(ToNumber(value))) > 0.0000000001)
                throw new CompileError("SEMANTIC", "S135", target.Line, target.Column, "Runtime int math requires an integer literal/expression or runtime int slot.");

            var amountNumber = ToNumber(value);
            if (amountNumber < int.MinValue || amountNumber > int.MaxValue)
                throw new CompileError("SEMANTIC", "S135", target.Line, target.Column, "Runtime int math literal must fit i32.");
            var amount = ((int)Math.Round(amountNumber)).ToString(CultureInfo.InvariantCulture);
            AddRuntimeAction(new RuntimeAction(op == "+" ? "runtime_int_add" : "runtime_int_sub", "", "static", amount, RuntimeSlotName(target)));
        }

        bool TryRuntimeIntSymbolOperand(ExprResult value, out string sourceName)
        {
            sourceName = "";
            const string prefix = "symbol(";
            if (!value.Repr.StartsWith(prefix, StringComparison.Ordinal) || !value.Repr.EndsWith(")", StringComparison.Ordinal))
                return false;

            var name = value.Repr.Substring(prefix.Length, value.Repr.Length - prefix.Length - 1);
            var token = new Token("STRING", name, 0, 0);
            var info = ResolveSymbol(token, "S137", n => $"Unknown runtime int symbol \"{n}\".");
            if (!info.IsRuntime || info.Type != "int")
                throw new CompileError("SEMANTIC", "S137", 0, 0, $"Runtime int math source \"{name}\" must be a runtime int slot.");
            sourceName = RuntimeSlotName(token);
            return true;
        }

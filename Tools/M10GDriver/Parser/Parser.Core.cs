...
static partial class Program
{
    sealed partial class Parser
    {
        ...
        readonly List<RuntimeFunction> _runtimeFunctions = new();

        List<RuntimeAction>? _runtimeActionCapture = null;

        string? _currentRuntimeFunctionName = null;

        int _runtimeWhileParseDepth = 0;

        bool _runtimeFunctionContainsReturn = false;

        string? _currentRuntimeFunctionReturnType = null;

        List<RuntimeLocal>? _currentRuntimeFunctionLocals = null;

        Dictionary<string, VarInfo>? _runtimeFunctionSavedLocalSymbols = null;

        Dictionary<string, RuntimeArrayInfo>? _runtimeFunctionSavedLocalArrays = null;

        Dictionary<string, RuntimeRecordInstance>? _runtimeFunctionSavedLocalRecords = null;

        Dictionary<string, RuntimeEnumInstance>? _runtimeFunctionSavedLocalEnums = null;

        Dictionary<string, RuntimeEnumArrayInfo>? _runtimeFunctionSavedLocalEnumArrays = null;
        ...
        readonly Dictionary<string, RuntimeIntArrayInfo> _runtimeIntArrays = new(StringComparer.Ordinal);

        readonly Dictionary<string, RuntimeArrayInfo> _runtimeArrays = new(StringComparer.Ordinal);

        readonly Dictionary<string, RuntimeRecordType> _runtimeRecordTypes = new(StringComparer.Ordinal);

        readonly Dictionary<string, RuntimeRecordInstance> _runtimeRecordInstances = new(StringComparer.Ordinal);

        readonly Dictionary<string, RuntimeRecordArrayInfo> _runtimeRecordArrays = new(StringComparer.Ordinal);

        readonly Dictionary<string, RuntimeEnumType> _runtimeEnumTypes = new(StringComparer.Ordinal);

        readonly Dictionary<string, RuntimeEnumInstance> _runtimeEnumInstances = new(StringComparer.Ordinal);

        readonly Dictionary<string, RuntimeEnumArrayInfo> _runtimeEnumArrays = new(StringComparer.Ordinal);

        int _runtimeArrayDispatchCounter = 0;

        int _runtimeSwitchCounter = 0;

        readonly Dictionary<string, List<Token>> _functions = new(StringComparer.Ordinal);

        readonly Dictionary<string, RuntimeFunction> _runtimeFunctionMap = new(StringComparer.Ordinal);

        readonly Dictionary<string, string> _runtimeParamAliases = new(StringComparer.Ordinal);

        readonly HashSet<string> _runtimeFunctionParamNames = new(StringComparer.Ordinal);

        readonly HashSet<string> _runtimeFunctionLocalNames = new(StringComparer.Ordinal);
        ...
        public Parser(List<Token> tokens)
        {
            ...
            [
                ...
                new StatementRule(() => CurrentWordIs("runtime") && PeekWord("while"), ParseRuntimeWhileStatement),
                new StatementRule(() => CurrentWordIs("runtime") && PeekWord("if"), ParseRuntimeIfStatement),
                new StatementRule(() => CurrentWordIs("runtime") && PeekWord("switch"), ParseRuntimeSwitchStatement),
                new StatementRule(() => IsKeyword("define") && PeekWord("enum"), ParseRuntimeEnumTypeDefinitionStatement),
                new StatementRule(() => IsKeyword("define") && PeekWord("runtime") && PeekWord("enum", 2) && PeekWord("array", 3), ParseRuntimeEnumArrayDefinitionStatement),
                new StatementRule(() => IsKeyword("define") && PeekWord("runtime") && PeekWord("enum", 2), ParseRuntimeEnumInstanceDefinitionStatement),
                new StatementRule(() => IsKeyword("set") && PeekWord("runtime") && PeekWord("enum", 2) && PeekWord("array", 3), ParseRuntimeEnumArraySetStatement),
                new StatementRule(() => IsKeyword("set") && PeekWord("runtime") && PeekWord("enum", 2), ParseRuntimeEnumSetStatement),
                new StatementRule(() => IsKeyword("define") && PeekWord("record"), ParseRuntimeRecordTypeDefinitionStatement),
                new StatementRule(() => IsKeyword("define") && PeekWord("runtime") && PeekWord("record", 2) && PeekWord("array", 3), ParseRuntimeRecordArrayDefinitionStatement),
                new StatementRule(() => IsKeyword("define") && PeekWord("runtime") && PeekWord("record", 2), ParseRuntimeRecordInstanceDefinitionStatement),
                new StatementRule(() => IsKeyword("set") && PeekWord("runtime") && PeekWord("record", 2) && PeekWord("array", 3), ParseRuntimeRecordArrayFieldSetStatement),
                new StatementRule(() => IsKeyword("set") && PeekWord("runtime") && PeekWord("record", 2), ParseRuntimeRecordFieldSetStatement),
                new StatementRule(() => CurrentWordIs("reset") && PeekWord("runtime") && PeekWord("record", 2), ParseRuntimeRecordResetStatement),
                new StatementRule(() => CurrentWordIs("copy") && PeekWord("runtime") && PeekWord("record", 2), ParseRuntimeRecordCopyStatement),
                new StatementRule(() => CurrentWordIs("fill") && PeekWord("runtime") && PeekWord("enum", 2) && PeekWord("array", 3), ParseRuntimeEnumArrayFillStatement),
                new StatementRule(() => CurrentWordIs("fill") && PeekWord("runtime"), ParseRuntimeArrayFillStatement),
                new StatementRule(() => CurrentWordIs("copy") && PeekWord("runtime") && PeekWord("enum", 2) && PeekWord("array", 3), ParseRuntimeEnumArrayCopyStatement),
                new StatementRule(() => CurrentWordIs("copy") && PeekWord("runtime"), ParseRuntimeArrayCopyStatement),
                new StatementRule(() => IsKeyword("define") && PeekWord("runtime") && (PeekKeyword("int", 2) || PeekKeyword("bool", 2) || PeekKeyword("string", 2)) && PeekWord("array", 3), ParseRuntimeArrayDefinitionStatement),
                new StatementRule(() => IsKeyword("set") && PeekWord("runtime") && (PeekKeyword("int", 2) || PeekKeyword("bool", 2) || PeekKeyword("string", 2)) && PeekWord("array", 3), ParseRuntimeArraySetStatement),
                new StatementRule(() => IsKeyword("set") && PeekWord("runtime") && (PeekKeyword("int", 2) || PeekKeyword("bool", 2) || PeekKeyword("string", 2)), ParseRuntimeSetStatement),
                new StatementRule(() => CurrentWordIs("toggle") && PeekWord("runtime") && PeekKeyword("bool", 2), ParseRuntimeToggleBoolStatement),
                new StatementRule(() => CurrentWordIs("break"), ParseRuntimeBreakStatement),
                new StatementRule(() => CurrentWordIs("continue"), ParseRuntimeContinueStatement),
                new StatementRule(() => CurrentWordIs("return"), ParseFunctionReturnStatement),

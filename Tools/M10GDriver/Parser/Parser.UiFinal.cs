...
static partial class Program
{
    sealed partial class Parser
    {
        ...
        void ParseUiEventStatementAfterWhen(bool apply, bool inIf)
        {
            ...
            var bodyLineCount = ReadUiMetadataBlock("when", "P250", "S253", "UI event block cannot be empty.");
            if (apply)
                _uiEvents.Add(new UiEvent(eventName, targetTok.Value, targetKind, bodyLineCount));
        }
        ...
        int ReadUiMetadataBlock(string blockName, string missingEndCode, string emptyCode, string emptyMessage)
        {
            var bodyLineCount = 0;
            var currentLine = -1;
            ...
            while (!CurrentIs("EOF") && !(IsKeyword("end") && PeekWord(blockName)))
            {
                ...
                if (!CurrentIs("NEWLINE") && Current.Line != currentLine)
                {

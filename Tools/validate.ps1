...
function Get-RelativePathSafe {
    ...
    try {
        ...
        $baseFull = $baseFull.TrimEnd([char[]]@([System.IO.Path]::DirectorySeparatorChar, [System.IO.Path]::AltDirectorySeparatorChar)) + [System.IO.Path]::DirectorySeparatorChar
...
function Test-ArqenNameTokenMatch {
    param(
        [string]$BaseName,
        [string]$Query
    )
    ...
    $q = $Query
    if ($q.EndsWith('.ps1', [System.StringComparison]::OrdinalIgnoreCase)) {
        $q = $q.Substring(0, $q.Length - 4)
    }

    $tokens = @($q -split '[_\-\s]+' | Where-Object { -not [string]::IsNullOrWhiteSpace($_) })
    if ($tokens.Count -eq 0) { return $false }

    $pos = 0
    foreach ($token in $tokens) {
        $idx = $BaseName.IndexOf($token, $pos, [System.StringComparison]::OrdinalIgnoreCase)
...
function Resolve-ArqenScript {
    ...
    if ($loose.Count -eq 0) {
        $loose = @(Get-ChildItem $base -Recurse -File -Filter "$Prefix*.ps1" | Where-Object { Test-ArqenNameTokenMatch $_.BaseName $Name })
    }

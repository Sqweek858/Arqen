...
function Get-ArqenRepoRoot {
    ...
    while ($null -ne $current) {
        if ((Test-Path (Join-Path $current.FullName "Docs\MILESTONES.md")) -and (Test-Path (Join-Path $current.FullName "Tools"))) {
    ...
    try {
        ...
        if (-not [string]::IsNullOrWhiteSpace($gitRoot)) { return $gitRoot }
    } catch {}
...
$target = Join-Path $RepoRoot "Tools\Common\new_command_skeleton.ps1"
if ($DryRun -and $Force) {

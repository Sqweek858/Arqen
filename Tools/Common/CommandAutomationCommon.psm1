function Get-ArqenRepoRoot {
    $current = [IO.DirectoryInfo]::new($PSScriptRoot)
    while ($null -ne $current) {
        $milestones = Join-Path $current.FullName "Docs\MILESTONES.md"
        $tools = Join-Path $current.FullName "Tools"
        if ((Test-Path $milestones) -and (Test-Path $tools)) {
    ...
    throw "Unable to locate Arqen repository root from $PSScriptRoot"
}
...
function Get-ArqenCommandSpecs {
    ...
    $root = Get-ArqenRepoRoot
    $files = @(Get-ChildItem (Join-Path $root "Tests\CommandTests\misc") -Filter "*.command.txt" -File | Sort-Object Name)
    if ($IncludeDrafts) {

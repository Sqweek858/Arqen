...
& (Join-Path (Split-Path -Parent $PSScriptRoot) "generate_keyword_registry.ps1") *> $null
& (Join-Path (Split-Path -Parent $PSScriptRoot) "generate_parser_rule_registry.ps1") *> $null
& (Join-Path (Split-Path -Parent $PSScriptRoot) "generate_command_test_index.ps1") *> $null
& (Join-Path (Split-Path -Parent $PSScriptRoot) "generate_command_status.ps1") *> $null
...
$runnerText = File-Text (Join-Path $root "Tools\run_all_tests.ps1")

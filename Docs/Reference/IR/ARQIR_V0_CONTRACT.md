...
## M30A DX12 UI overlay runtime subset

M30A consumes existing UI IR records in the DX12 lowerer:

- `UI_OBJECT|type=...|name=...`
- `UI_SET|target=...|property=content/checked|...`
- `UI_LAYOUT|target=...|property=x/y/width/height|unit=px|...`
- `STYLE|target=...|state=default|property=background color/foreground color/border color/border size/opacity|...`
- `UI_EVENT|event=clicked|target=...|...`

The IR record shape is unchanged from M19; M30A only adds DX12 consumption for a strict runtime subset.

## M30B/M30C DX12 UI runtime bridge markers

The DX12 lowerer can consume existing M19 UI metadata for overlay rendering:
...
M30B/M30C generated config markers include:

- `ARQEN_M30B_UI_LAYOUT_HYGIENE`
- `ARQEN_M30B_TEXT_CLIPPING`
- `ARQEN_M30B_BUTTON_TEXT_CENTERING`
- `ARQEN_M30B_Z_INDEX_DRAW_ORDER`
- `ARQEN_M30C_UI_PARENT_CLIP_BRIDGE`
- `ARQEN_M30C_PARENT_RELATIVE_LAYOUT`
- `ARQEN_M30C_BASIC_PANEL_CLIP`
- `ARQEN_M30D_UI_CLICK_EVENT_BRIDGE`

M30D's event bridge is partial: `UI_EVENT` currently exposes event/target/body-line count, not executable event-body actions.

## M30D/M31A/M31B UI runtime bridge

`UI_EVENT` may include a compact `body=` field preserving the text of the metadata block. The DX12 lowerer uses this to map known clicked event bodies to runtime actions. Existing UI resource metadata (`UI_RESOURCE`, `UI_RESOURCE_USE`) is consumed as `ARQEN_M31B_UI_RESOURCE_METADATA_BRIDGE` / `M31B_UI_RESOURCE_METADATA_BRIDGE` runtime markers. Full generic event execution and real font/texture engines remain out of scope.
...
## M39 function IR blocks

M39 adds no new public syntax, but the IR can now preserve function bodies:

```txt
ACTION|id=fn_0_act_0|op=print_stdout|value_kind=static|value=boot|target=
FUNCTION|name=boot|actions=fn_0_act_0
ACTION|id=act_0|op=function_call|path=|value_kind=static|value=|target=boot
ENTRY|actions=act_0,act_1
```

`FUNCTION` blocks define action id lists that are emitted as backend routines. `function_call` transfers control to the function routine and returns to the caller. M39 has no params, returns, local scope, or recursion.

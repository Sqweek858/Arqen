...
Compiler-integrated DX12 support is not implemented yet. This file defines the contract that must exist before the backend is allowed to become `supported` in `Docs/Reference/Backends/WindowsX64PE_Config/capabilities_v0.txt`. M20A adds native bridge source only; the public compiler feature remains unsupported.
